        ...
        <View>
            {
                (dropdown && search)
                    ...
                    <TouchableOpacity style={[styles.wrapper, boxStyles]} onPress={() => { if (!dropdown) { Keyboard.dismiss(); slidedown() } else { slideup() } }}>
                        <Text style={[{ fontFamily }, inputStyles]}>{(selectedval == "") ? (placeholder) ? placeholder.length > 5 && Dimensions.get('window').width < 550 ? placeholder.slice(
                            0, 6) + '...' : placeholder
                            : 'Select option' : selectedval}</Text>
                        {
                            (!arrowicon)
                                ?
                                <Image
