...
import { lightBlue, errorRed, inputGrey, lightGreen, white, buttonGreen } from '../utils/Color';
import { onBlurErrorAge, onBlurErrorEmail, onBlurErrorFirstName, onBlurErrorGender, onBlurErrorLastName, onBlurErrorPhoneNo, onChangeAge, onChangeEmail, onChangeFirstName, onChangeLastName, onChangePhoneNo } from '../utils/Validations';
...
const RequestAppointment = ({ route }) => {
    ...
    const [selectedDate, setSelectedDate] = useState<String>(route?.params?.userEmailData.appointmentDate ? route?.params?.userEmailData.appointmentDate : '')
    ...
    const [userAppointmentDetails, setUserAppointmentDetails] = useState({
        ...
        lastName: route?.params?.userEmailData.lastName ? route?.params?.userEmailData.lastName : '',
        age: route?.params?.userEmailData.age ? route?.params?.userEmailData.age : 0,
        gender: route?.params?.userEmailData.gender ? route?.params?.userEmailData.gender : '',
        email: route?.params?.userEmailData.email ? route?.params?.userEmailData.email : '',
        appointmentDate: route?.params?.userEmailData.appointmentDate ? route?.params?.userEmailData.appointmentDate : '',
        slotTime: route?.params?.userEmailData.slotTime ? route?.params?.userEmailData.slotTime : '',
        phoneNo: route?.params?.userEmailData.phoneNo ? route?.params?.userEmailData.phoneNo : ''
    })
    ...
    const handleConfirm = (date: any) => {
        ...
        setSelectedDate(dt1)
        setUserAppointmentDetails({ ...userAppointmentDetails, appointmentDate: dt1 })
    ...
    useEffect(() => {
        if (route?.params?.userEmailData.slotTime) {
            const id: number | undefined = timeData.find((data) => data.date === route?.params?.userEmailData.slotTime)?.id;
            selectedSlot(id)
            // 
        }
    }, [])
    ...
    function selectedSlot(id: Number | undefined) {
        const updatedTimeData = timeData.map((e) => {
            if (e.id === id) {
                setUserAppointmentDetails({ ...userAppointmentDetails, slotTime: e.date })
    ...
    const appointData = useSelector((state: any) => state.reducer.userAppointData)
    ...
    const handleSubmit = () => {
        ...
        if (newFormValidation.errCount === 0 && !newFormValidation.emailError && !route?.params?.userEmailData) {
    ...
    useEffect(() => {
        if (userAppointmentDetails.appointmentDate !== '') {
            setAppointDateError(false);
            appointmentFormValidation.errCount = Math.max(appointmentFormValidation.errCount - 1, 0);
    ...
    return (
        <>
            <ErrorPopUp errCount={appointmentFormValidation.errCount} errMsg={appointmentFormValidation.errMsg} />
            <ScrollView bounces={false}>
                <KeyboardAwareScrollView>
                    ...
                    <View style={styles.container}>
                        ...
                        <Text style={styles.textLabel}>Name <Text style={styles.star}>*</Text></Text>
                        <View style={styles.parentInput}>
                            <View style={styles.inputContainer}>
                                <CustomTextInput style={styles.input} onBlur={() => onBlurErrorFirstName(userAppointmentDetails, setAppointFormValidation, appointmentFormValidation)} onChange={() => onChangeFirstName(userAppointmentDetails, setAppointFormValidation, appointmentFormValidation)} placeholder={''} errorText={appointmentFormValidation.firstNameErrTxt} value={userAppointmentDetails.firstName} setUserDetails={setUserAppointmentDetails} userDetails={userAppointmentDetails} name='firstName' errorMsg={appointmentFormValidation.firstNameErr} />
                                <Text style={styles.bottomLabel}>First Name</Text>
                            </View>
                            <View style={styles.inputContainer}>
                                <CustomTextInput style={styles.input} placeholder={''}
                                    errorText={appointmentFormValidation.lastNameErrTxt} onBlur={() => onBlurErrorLastName(userAppointmentDetails, setAppointFormValidation, appointmentFormValidation)} onChange={() => onChangeLastName(userAppointmentDetails, setAppointFormValidation, appointmentFormValidation)} value={userAppointmentDetails.lastName} setUserDetails={setUserAppointmentDetails} userDetails={userAppointmentDetails} name='lastName' errorMsg={appointmentFormValidation.lastNameErr}
                                />
                                <Text style={styles.bottomLabel}>Last Name</Text>
                            </View>
                        </View>
                        <Text style={styles.textLabel}>Patient Gender <Text style={styles.star}>*</Text></Text>
                        <View style={styles.genderContainer}>
                            ...
                            <SelectDropdown
                                data={gender}
                                onSelect={(selectedItem, index) => setUserAppointmentDetails({ ...userAppointmentDetails, gender: selectedItem.value })
                                }
                                defaultValueByIndex={route?.params?.userEmailData.gender ? route?.params?.userEmailData.gender === 'Male' ? 0 : 1 : undefined}
                                    ...
                                    return (
                                        <View style={styles.dropdownButtonStyle}>
                                            ...
                                            <Text style={styles.dropdownButtonTxtStyle}>
                                                {(selectedItem && selectedItem.value) || 'Select your Gender'}
                                            </Text>
                                ...
                                renderItem={(item, index, isSelected) => {
                                    return (
                                        <View style={{ ...styles.dropdownItemStyle, ...(isSelected && { backgroundColor: '#cacdcf' }) }}>
                                ...
                                onBlur={() => onBlurErrorGender(userAppointmentDetails, setAppointFormValidation, appointmentFormValidation)}
                        ...
                        <Text style={styles.textLabel}>Age <Text style={styles.star}>*</Text></Text>
                        <CustomTextInput style={[styles.input]} onChange={() => onChangeAge(userAppointmentDetails, setAppointFormValidation, appointmentFormValidation)} onBlur={() => onBlurErrorAge(userAppointmentDetails, setAppointFormValidation, appointmentFormValidation)} placeholder='ex: 23' errorText={appointmentFormValidation.ageErrTxt} type={'number-pad'} value={userAppointmentDetails.age} setUserDetails={setUserAppointmentDetails} userDetails={userAppointmentDetails} name='age' errorMsg={appointmentFormValidation.ageError} />
                        <Text style={styles.textLabel}>Phone Number <Text style={styles.star}>*</Text></Text>
                        <CustomTextInput style={styles.input} onChange={() => onChangePhoneNo(userAppointmentDetails, setAppointFormValidation, appointmentFormValidation)} onBlur={() => onBlurErrorPhoneNo(userAppointmentDetails, setAppointFormValidation, appointmentFormValidation)} placeholder='(+91) 0000-0000' type={'number-pad'}
                        ...
                        <View>
                            <Text style={styles.textLabel}>Select an Appointment Date <Text style={styles.star}>*</Text></Text>
                            <View style={{ flexDirection: 'row' }}>

                                <TouchableOpacity
                                    style={[styles.calendar, { borderColor: appointDateError ? errorRed : inputGrey }]}
                                    onPress={showDatePicker}
                                >
                                    <Text>{selectedDate ? selectedDate : defaultDate}</Text>
                                    <View style={styles.calendarIcon}>
                                        <FontAwesome name='calendar' size={18} color={'grey'} onPress={showDatePicker} />
                                    </View>
                                    <DateTimePickerModal
                        ...
                        <Text style={styles.textLabel}>{day}, {month} {date}</Text>

                        <View style={styles.appointSlot}>
                            {timeData.map((data, index) => {
                                return (
                                    <>
                                        <Pressable key={data.id} onPress={() => selectedSlot(data.id)} style={[styles.appointBox, { backgroundColor: data.isSelected ? lightBlue : 'transparent' }]}>
                                            <Text style={[styles.appointText, { color: data.isSelected ? white : '' }]}>{data.date}</Text>
                                        </Pressable>
                                    </>
                                )
                            })
                        ...
                        </View>
                        {
                            appointSlotError ? (
                                <>
                                    <View style={styles.errorMessage}>
                                        <MaterialIcons name='error-outline' size={15} color={errorRed} />
                                        <Text style={styles.errorText}>Please select appointment slot.</Text>
                                    </View>
                                </>
                            ) : (
                                <></>
                                    ...
                                    <>
                                        <View key={data.id} style={styles.selectSlot}>
                                            <View key={data.id} style={styles.dateTime}>
                                                <FontAwesome5 style={{ margin: 5, marginTop: 10 }} name='calendar-check' size={24} color={white} />
                                                <View>
                                                    <Text style={[styles.selectText]}>Selected Time</Text>
                                                    <Text style={styles.selectText2}>{data.date}, {day}, {month} {date}</Text>
                                                </View>
                                            </View>
                                            <TouchableOpacity style={styles.cancelBtn} onPress={() => cancelSelect(data.id)}>
...
const getStyles = (genderErr: any) => StyleSheet.create({
    container: {
        flex: 1,
